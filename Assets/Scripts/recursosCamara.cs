...
public class recursosCamara : MonoBehaviour 
{
	...
	public void setPosicionJugador(){
		GameObject.FindWithTag ("jugador").GetComponent<Transform>().position = posJugador;
	}
	...
	void OnGUI()
	{
		...
		GUI.Label (new Rect (120, 10, 100, 20), "Botellas: " + botellas);
		...
		if (viveros = 1) {
			imprimeObjetivoLogrado("Enhorabuena! Has logrado crear tu primer vivero!");
		}
	}

	public void imprimeObjetivoLogrado(string s){
		GUI.Box (new Rect(Screen.width / 2, Screen.height / 2,300,150), s);
		}
